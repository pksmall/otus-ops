...
NEW_WORD_FOUND = prometheus_client.Histogram('crawler_new_word_found', 'How much time it took to parse a page')

def connect_db():
    try:
        mngport = int(getenv('MONGO_PORT', '27017'))
        mnghost = getenv('MONGO', 'mongo')
        uri = "mongodb://%s:%s@%s:%d" % ("root", getenv('MONGO_PASSWORD', 'mongo-root-password'), mnghost, mngport)
        db = MongoClient(uri)
...
mqport = int(getenv('RMQ_PORT', '31706'))
